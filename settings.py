...
В отличие от compare.py и converter.py, эти окна очень плотно
завязаны на состояние главного окна приложения (язык, тема, сохранённые
настройки, шаблон имени файла и т.д.). Используется тот же приём,
что и для Compare: главное окно (App) передаётся явным
